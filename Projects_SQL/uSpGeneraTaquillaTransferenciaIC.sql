ALTER  PROCEDURE [dbo].[uSpGeneraTaquillaTransferenciaIC]                                    
/*---------------------------------------------                                    
...
declare @codimpuestox int    
declare @codimpuesto int    
declare @montoimpuestox real                               
...
declare CURSOR_ENTRYDATA  cursor for                                      
    --// SELECT de consulta de los datos de la remesa                                      
      ...
      SELECT c.operacion, a.auxiliar_sol AS 'Auxiliar',                 
          isnull(b.Nombres,'')+' '+isnull(b.segundonombre,'')+isnull(b.Apellido1,'')+' '+isnull(b.Apellido2,'') as 'Cliente',                                     
          c.beneficiario,                 
          ...
          d.simbolo,Auxiliar_Benef, c.tasa, c.fixing, mi.codimpuesto, mi.totalimpuesto                                 
   ...
   inner join movimp mi on (mi.operacion = c.operacion)    
   ...
   WHERE (Instrumento = 'TF') AND (a.Estado in ('C','H'))     
   AND dbo.uFnDiasLaborales(c.fecha_bcv,getdate(),mo.codagencia)<= @diaspermitido                               
   AND (c.operacion =@OperacionB)    
...
into @Operacion,@Auxiliar,@Cliente,@Beneficiario,@PaisBenef,@DirBenef,@MontoBenef,@Oprecdep,@montorecibido,@fecha_ger,                
     @solicitud,@Fecha_Aprob_Cadivi,@TipoRemesa,@Fecha_Caja,@destino,@simbolo,@auxiliar_benef,@tasa,@fixing,@codimpuestox,@montoimpuestox                                    
while @@fetch_status = 0                                      
   begin      
       
        SET @MONTOIMPUESTO = @montoimpuestox    
        SET @COMISION_MONEYGRAM = @montoimpuestox      
        SET @codimpuesto =   @codimpuestox                           
    fetch next from CURSOR_ENTRYDATA                                      
    into @Operacion,@Auxiliar,@Cliente,@Beneficiario,@PaisBenef,@DirBenef,@MontoBenef,@Oprecdep,@montorecibido,@fecha_ger,                
         @solicitud,@Fecha_Aprob_Cadivi,@TipoRemesa,@Fecha_Caja,@destino,@simbolo,@auxiliar_benef,@tasa,@fixing,@codimpuestox,@montoimpuestox    
   end                                      
...
deallocate CURSOR_ENTRYDATA                                      
  ...
  declare @CodSwif varchar(100)                
      ...
      select '1' as divisa, t.codbanco,t.Direccion,t.abba,t.codcuenta,b.direccion_benef,PAI_NOMBRE as PaisBenef,                
       ...
       tb.Estado as EstadoInt,tb.Ciudad as CiudadInt,tb.CodPostal as CodPostalInt,t.Swift                
      from tblrusadtransferencia t                 
      inner join trusadbeneficiario b on (b.auxiliarbenef =t.auxiliarbenef and b.solicitud=t.solicitud)                
...
--------------- ************************************************ ----------------------                                    
   ...
   DECLARE @OperacionTransferCAJA VARCHAR(30)                                    
   SET @OperacionTransferCAJA = @OP                                    
   ...
   set @ControlCambio  = (Select top 1 valor from parametros where clave = 'CONTROLDECAMBIO')                                    
    ...
    --// SELECT de consulta de los datos de la remesa                                      
        ...
        where (t.operacion=@OperacionTransferCAJA) and r.estado in ('A','X')                
